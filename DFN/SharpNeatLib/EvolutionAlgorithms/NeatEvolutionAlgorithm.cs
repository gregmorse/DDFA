...
namespace SharpNeat.EvolutionAlgorithms
{
        ...
        where TGenome : class, IGenome<TGenome>
    {
        ...
        public List<NoveltyArchive<TGenome>> noveltyArchives = new List<NoveltyArchive<TGenome>>();
        ...
        public NeatEvolutionAlgorithm()
        {
            ...
            //DFN: Make sure novelty happens across ALL of our ea params -- consistently
            _eaParamsComplexifying.UseNovelty = _eaParams.UseNovelty;
            _eaParamsSimplifying.UseNovelty = _eaParams.UseNovelty;
        }
                                      ...
                                      IComplexityRegulationStrategy complexityRegulationStrategy)
        {
            ...
            //DFN: Make sure novelty happens across ALL of our ea params
            _eaParamsComplexifying.UseNovelty = _eaParams.UseNovelty;
            _eaParamsSimplifying.UseNovelty = _eaParams.UseNovelty;
        }
        ...
        private void Initialize()
        {
            ...
            //DFN: if novelty -- do a novelty eval here
            if (_eaParams.UseNovelty)
            {
                //loop through applying our novelty archive calculations
                evaluateNovelty();
            }
        ...
        public void AddNoveltyArchive(NoveltyArchive<TGenome> archive)
        {
            //we add the archive for novelty evaluation
            noveltyArchives.Add(archive);
        }

        void evaluateNovelty()
        {
            //loop throuhg our archives sending the current population for evaluations
            for (int i = 0; i < noveltyArchives.Count; i++)
            {
                //this will handle evaluating the current population (and adjusting fitness values to novelty if necessary)
                noveltyArchives[i].evaluatePopulation(this.GenomeList, true);                                                    
            }
            System.IO.File.AppendAllText("archivesize.txt", System.DateTime.Now.ToString() + " : " + noveltyArchives[noveltyArchives.Count-1].ArchiveList.Count + Environment.NewLine);
        }
        ...
        {           
            ...
            //DFN: Here me always make a call to the novelty archives after eval -- check if we are running novelty
            if (_eaParams.UseNovelty)
            {
                //loop through applying our novelty archive calculations
                evaluateNovelty();
