...
namespace SharpNeat.Decoders.HyperNeat
{
    ...
    public class DefaultNodeSetMappingFunction : INodeSetMappingFunction
    {
        ...
        /// </summary>
        public IEnumerable<SubstrateConnection> GenerateConnections(int layer, SubstrateNodeSet srcNodeSet, SubstrateNodeSet tgtNodeSet)
        {
            //DFN: How many nodes in the output, if only one -- we don't need 4 dimensions, only two :)
            bool targetSetSingleNode = (tgtNodeSet.NodeList.Count == 1);

            //DFN: Adding concept of Justin's multimodal setup
                ...
                for(int i=0; i<count; i++)
                {
                    ...
                    for(int j=0; j<i; j++)
                    {
                        if(_testNodePair(srcNode, nodeList[j])) {
                            yield return new SubstrateConnection(srcNode, nodeList[j], layer, targetSetSingleNode);
                        }
                    }

                    //  Test for local recurrent connection.
                    if(_allowLocalRecurrentConnections && _testNodePair(srcNode, srcNode)) {
                        yield return new SubstrateConnection(srcNode, srcNode, layer, targetSetSingleNode);
                    }
                ...
                foreach(SubstrateNode srcNode in srcNodeSet.NodeList)
                {
                    foreach(SubstrateNode tgtNode in tgtNodeSet.NodeList)
                    {
                        if(_testNodePair(srcNode, tgtNode)) {
                            yield return new SubstrateConnection(srcNode, tgtNode, layer, targetSetSingleNode);
                        }
                    }
                }
            }
        ...
        /// </summary>
        public int GetConnectionCountHint(int layer, SubstrateNodeSet srcNodeSet, SubstrateNodeSet tgtNodeSet)
        {
            ...
            int count = 0;
            foreach (SubstrateConnection conn in GenerateConnections(layer, srcNodeSet, tgtNodeSet))
            {
                count++;
            }
            return count;
