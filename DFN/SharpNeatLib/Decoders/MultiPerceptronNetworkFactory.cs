...
namespace SharpNeat.Decoders
{
    ...
    /// </summary>
    public class MultiPerceptronNetworkFactory
    {
        ...
        /// </summary>
        public static MultiPerceptron CreateAcyclicMultiPerceptronNetwork(INetworkDefinition networkDef)
        {
            ...
            //DFN: Wow. I think this function in SharpNEAT 2.0 was actually remapping activation functions improperly
            //it's a little complicated to explain, but it was pulling the remapped function from the wrong location -- confirmed on a leo seed. 
            //You are supposed to map old ixs to new ixs based on depth, then pull act function from the original, and set to the mapped
            //the previous function would map news to old, and pull activation from the newly mapped -- clearly an error
            for(int i=0; i<nodeCount; i++) 
            {
                NodeInfo nodeInfo = nodeInfoByDepth[i];
                int definitionIdx = nodeInfo._definitionIdx;
                if (nodeList[definitionIdx].NodeType == NodeType.Output) // BP Sigmoids for the output nodes
                    nodeActivationFnArr[i] = new SharpNeat.Network.BipolarSigmoid();
                else
        ...
        //=== Create array of FastConnection(s). 
            ...
            for(int currDepth=0; currDepth < netDepth; currDepth++)
            {
                ...
                return new MultiPerceptron(nodeActivationFnArr, nodeAuxArgsArray, fastConnectionArray, layerInfoArr, outputNeuronIdxArr,
        ...
        /// Creates a AcyclicNetwork from an INetworkDefinition.
        /// </summary>
        public static OpenAcyclicNetwork CreateAcyclicOpenNetwork(INetworkDefinition networkDef)
        {
            Debug.Assert(!CyclicNetworkTest.IsNetworkCyclic(networkDef), "Attempt to decode a cyclic network into a FastAcyclicNetwork.");
            ...
            return new OpenAcyclicNetwork(nodeActivationFnArr, nodeAuxArgsArray, fastConnectionArray, layerInfoArr, outputNeuronIdxArr,
                                      nodeCount, networkDef.InputNodeCount, networkDef.OutputNodeCount);
        }
        ...
        class NodeDepthComparer : IComparer<NodeInfo>
        {
            ...
            public int Compare(NodeInfo x, NodeInfo y)
            {
                ...
                if (x._nodeDepth != y._nodeDepth)
                    return x._nodeDepth - y._nodeDepth;
                else //if equal depth, sort based on nodeID -- lower first
                    return (int)x._nodeId - (int)y._nodeId;
            }
        }
